...
def computeOSQPStep(weights, slope, y_shift, active_set: List[ExtremalPoint], hesse: HesseMatrix, params) -> tuple[np.array, np.array, np.array]:
	...
	additional_ones[:len(active_set)] = np.ones_like(additional_ones[:len(active_set)])
	...
	q = - vectorStandardInner + reg #additional_ones
	l = np.zeros_like(x)
	for idx in range(len(active_set), n):
		l[idx] = -np.inf
	u = np.ones_like(x) * np.inf
	A = sparse.identity(n)
	prob.setup(P, q, A, l, u, verbose=False, eps_abs=1e-10, eps_prim_inf=1e-10, eps_dual_inf=1e-10)
	#prob.warm_start(x=x)
	res = prob.solve()
	x = res.x
