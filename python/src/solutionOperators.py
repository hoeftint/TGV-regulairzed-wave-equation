...
def solveStateEquation(control: List[fem.Function], params) -> List[fem.Function]:
    if params.fdType == 'backward-euler':
        return solveStateEquationBackwards(control, params)
    elif params.fdType == 'mid-rule':
        return solveStateEquationBackwards(control, params)
    raise Exception('Unknown finite difference solver')

def solveAdjointEquation(control: List[fem.Function], params) -> List[fem.Function]:
	control_rev = []
	control_rev[:] = list(reversed(control))
	return list(reversed(solveStateEquation(control_rev, params)))[:]

def solveStateEquationBackwards(control: List[fem.Function], params) -> List[fem.Function]:
...
def solveStateEquationMid(control: List[fem.Function], params) -> List[fem.Function]:
	solution = []
