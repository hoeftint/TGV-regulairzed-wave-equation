...
def computeObjective(x, active_set, standard_states, params):
    ...
    for i, func in enumerate(active_set):
        sum_points += (x[i] * (func.type * params.beta - params.alpha * (func.type- 1)))
...
def computeSSNStep(weights, slope, y_shift, active_set: List[ExtremalPoint], hesse: HesseMatrix, params) -> tuple[np.array, np.array, np.array]:
    ...
    q_k = u_k + 1/params.newton_c * np.array([1. if k < len(active_set) else 0. for k in range(n)])
    P_c = computeProx(q_k, active_set, params)
    obj = computeObjective(P_c, active_set, standard_states, params)
    obj_qk = computeObjective(q_k, active_set, standard_states, params)
    #print('Objective: ', obj)
    #print(-1, ': Iteration: ', P_c)
    for k in range(params.maxNewtonSteps):
        Df = computeDifferential(P_c, hesse, vectorStandardInner)
        DP_c = computeProxDifferential(q_k, active_set, params)
        G = params.newton_c * (q_k - P_c) + Df
        DG = params.newton_c * (np.identity(n) - DP_c) + np.matmul(hesse.matrix, DP_c)
        dq = scipy.linalg.solve(DG, -G, 'pos')
        q_k = q_k + dq
        P_c = computeProx(q_k, active_set, params)
        obj_old = obj_qk
        obj_qk = computeObjective(q_k, active_set, standard_states, params)
        obj_x = computeObjective(P_c, active_set, standard_states, params)
        if abs(obj - obj_old) < 1e-16 and k > 1:
            break
        print(k, ': Objective: ', obj_x, 'Objective raw', obj_qk)
        print(k, ': Iteration: ', P_c, 'Iteration raw', q_k)
